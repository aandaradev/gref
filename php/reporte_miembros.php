...
<html>
	...
	<body>
		...
		<br>
		<form name="frmReporte" action="reporte_M.php" method="POST">
			<table border="1" cellspadding="0" cellspacing="0" align="center">
				...
				<?php
					...
					$cnx = conectar();
					$sql = "SELECT CodGref,CedId,Apellidos,Nombres,Tlf,Email,Estado FROM personal WHERE Estado='". $_POST['estado']."' ORDER BY CodGref";
					$consulta = mysql_query($sql);
					while(list($codigo,$ci,$apellidos,$nombres,$tlf,$email,$estado) = mysql_fetch_array($consulta)){
						echo"<tr>";
							...
							echo"<td><div>$tlf<input type='hidden' name='tlf' value='$tlf'></div></td>";
							echo"<td><div>$email<input type='hidden' name='email' value='$email'></div></td>";
							echo"<div><input type='hidden' name='estado' value='$estado'></div>";
						echo"</tr>";
					}
